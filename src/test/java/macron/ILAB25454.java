...
public class ILAB25454 {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		WebElement Table = driver.findElement(By.id("quote_item_table"));
		
		List <WebElement> rows = Table.findElements(By.tagName("tr"));
		List <WebElement> Col = Table.findElements(By.tagName("th"));
		int j= Col.size();
		System.out.println("the number of column are " +j);
		for( WebElement i:Col ) {
			System.out.println(i.getText());
		}
		Thread.sleep(3000);
		int i = rows.size();
		System.out.println("the number of rows are" +i );
		driver.findElement(By.id("quotation_terms")).sendKeys("the disclaimer should be working fine");
		Thread.sleep(2000);
		
		driver.findElement(By.id("save_quotation")).click();
		Thread.sleep(2000);
		WebElement a = driver.findElement(By.xpath("//div[@class='messages_container']"));
		System.out.println("the text of the tooltip is" + a.getText());
		...
		Thread.sleep(10000);
		driver.findElement(By.xpath("//table[@class='ui table selectable compact celled']/tbody//td[@class='action']/a[1]")).click();
		driver.switchTo().alert().accept();
		WebElement b = driver.findElement(By.xpath("//div[@class='messages_container']"));
		System.out.println("the text of the tooltip is" + b.getText());
		driver.close();
	}
